				...
				<RightHeader>
					{isLoggedIn ? (
						<ProfileButton onClick={handleNavClick("/mypage")}>
							...
							<ProfileText>{username}님</ProfileText>
						</ProfileButton>
					) : (
						<ProfileText>로그인하세요</ProfileText>
					)}
					{isLoggedIn ? (
			...
			<SecondRow>
				<Navigation>
					...
					<Nav
						...
						active={activeNav === "/register"}
					>
						물품 등록
					</Nav>
					<Nav
						onClick={handleNavClick("/health")}
						active={activeNav === "/health"}
					>
						건강 AtoZ
					</Nav>
					<Nav
						onClick={handleNavClick("/goodsDetail")}
						active={activeNav === "/nearby"}
					>
						어디GO
					</Nav>
					<Nav
						onClick={handleNavClick("/chatting")}
						active={activeNav === "/chatting"}
					>
						채팅
					</Nav>
				</Navigation>
				<Search>
					<SearchInput
						type="text"
						placeholder="찾는 운동용품 또는 운동종목이 있나요?"
						value={searchValue}
